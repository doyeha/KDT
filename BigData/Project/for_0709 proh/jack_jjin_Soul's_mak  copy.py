...
# 1조 ~ 8조 4명씩 뽑기. 지금은 4명 뽑기
def get_first_card():        
    while len(Team1)<=4:
        TeamMate = pick_member()
        # print(f"1번 카드 {card}")
        if check_card(card) == True:
            dealer_card.append(card)

def check_card(picked_member): # 이미 뽑힌 조원인지 확인
    if picked_member not in AllTeam: # 1~8조를 묶은 "팀" 자체에 있는지 확인
        return True
    else:
        return False

def pick_member():    # 카드 랜덤 뽑기. 튜플 키로 카드번호 반환.
    ran = rand.randint(1,32)
    picked_member = KDT_member[ran]
    return picked_member  # 리스트 index 0은 키 / 1은 카드번호
