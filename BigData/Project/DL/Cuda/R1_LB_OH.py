...
from torchmetrics.classification import F1Score,BinaryAccuracy, BinaryF1Score, BinaryConfusionMatrix, BinaryRecall
...
class BinClaModel(nn.Module):
    def __init__(self):
        ...
        self.hid_layer1 = nn.Linear(10,20)
        self.hid_layer2 = nn.Linear(20,25)
        self.hid_layer3 = nn.Linear(25,5)
    ...
    def forward(self,x):
        ...
        return F.torch.sigmoid(self.out_layer(y)) # 체포 여부 이진분류니까!
...
for epoch in range(EPOCH):
    ...
    for featureTS, targetTS in trainDL :
        ...
        # 성능평가 계산
        Recall = BinaryRecall().to(DEVICE)(pre_y, targetTS)
        # 방법2 : score = F1Score(task='binary')(pre_y, targetTS)
        score_total += Recall.item()
...
axes[0].plot(range(1, len(LOSS_HISTORY[0])+1), LOSS_HISTORY[0][:len(LOSS_HISTORY[0])], label='Train')
axes[0].plot(range(1, len(LOSS_HISTORY[0])+1), LOSS_HISTORY[1][:len(LOSS_HISTORY[0])], label='Val')
axes[0].grid()
axes[0].legend()
...
axes[1].plot(range(1, len(SCORE_HISTORY[0])+1), SCORE_HISTORY[0][:len(SCORE_HISTORY[0])], label='Train')
axes[1].plot(range(1, len(SCORE_HISTORY[0])+1), SCORE_HISTORY[1][:len(SCORE_HISTORY[0])], label='Val')
axes[1].grid()
axes[1].legend()
