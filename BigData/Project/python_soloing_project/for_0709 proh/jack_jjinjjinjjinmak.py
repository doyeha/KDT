...
"""
연승할 수록 . 판돈 2배씩, 따는 것도 2배.
win == 가중치
첫판    500 -> win : 0
두번째 1000 -> win : 1 // 500
세번째 2000 -> win : 2 // 1000

game_money = game_money+win*500
...
while game_money >= 500:
    ...
    if lose == 0 and win == 0:  # 완전 쌩 처음.
        print(f" KD 도박장에 오신 것을 환영합니다. 저희의 대표게임은 K_Jack입니다. 왤컴 선물로 게임머니 1000을 드렸습니다.\n K_Jack 시작하시겠습니까? \n 현재 게임 머니 : {game_money} \n 한 게임 당 게임 머니 500이 소모됩니다. ")
    elif lose <= 0 and win > 0: # 연승 중
    ...
    if lose<=0 and win > 0:
        print("더 큰 머니를 걸고, 도전 해보시겠습니까? (y/n)", end=" ")
    elif lose > 0:
        print("정말 다시 도전하십니까?")
    else: 
    ...
    if get_input in ["예", "Yes", "Y", "네", "y"]:
        game_money -= 500+win*500
        print("너무 중독되지 않으시기를, 행운을 빕니다." if dobak==True else "")
        print(f"\n\n게임을 시작합니다! \n게임 머니 : {game_money}")
    # 설명 선택
    elif get_input in ["e", "E"]:   
        print(explains, end=" : ")
        new_input = input() # 다시 시작 여부 질문
    ...
    if check == 0:  # func 하기엔 continue 걸림.
        ...
        if game_money <= 0:
            ...
            print("\n\n\n\n\n\n\n\n") # 화면 정리용
            continue
            # if check in ["플 21 초과", "딜 21 초과", "딜 힛", "플 힛"]
            continue
    else:
        print(f"오류 발생.")
        break
    ...
    while check == 0: # 카드 더 받는가. 근데, over체크에서 아무도 over안해야함.
        ...
        if more_card in ["Y", "y", "네", "예"]:
            ...
            if check == 0:  # func 하기엔 continue 걸림.
                ...
                lose += 1
                if game_money <= 0:
                    print("저희가 제공해드린 기본 머니를 다 사용하셨군요. 소유하신 것이라도 처분해 새로운 머니를 마련해보시겠습니까?\n진심이시라면 이곳을 나가 10분 거리에 중고물품 판매장이 있습니다.\n도박장 근처에 기생하는 곳인 만큼 값을 후하게 주지는 않으니 기대 마시기를.")
                    break
            else:
                print(f"오류 발생.")
                break
            ...
            if not check == 0:  # 딜러와 플레이어 중 한 사람이라도 21을 달성했거나 22를 초과했으므로 게임 끝. continue로 while 시작점 복귀
                if game_money <= 0:
                    print("머니를 모두 잃으신 겁니까? 실력인지 운인지, 유감일 따름입니다.")
                    break
                else:
                    print(f"더 도전해보시겠습니까? 현재 게임머니 : {game_money}", end=" ")
                    new_game_question = input()
                    if not new_game_question in ["y", "Y"]:
                        print("즐거운 시간이었기를")
                        regame = False
        ...
        # 카드 더 안받겟다고 하면 바로 체크. 
                ...
                if check == 0:  # func 하기엔 continue 걸림.
                    ...
                    if game_money <= 0:
                        ...
                        print(f"한 판 더 어떠신가요? 현재 게임머니 : {game_money}", end=" ")
                        new_game_question = input()
                        if not new_game_question in ["y", "Y"]:
                            print("즐거운 시간이었기를")
                            break # game = False
