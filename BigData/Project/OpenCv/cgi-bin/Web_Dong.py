     ...
     <body>
      <div class="container">
        <form enctype="multipart/form-data" method="post">
          <p>텍스트 입력: <textarea name="text" rows="10" cols="40">{text}</textarea></p>
...
def detectRANK(image_field, model):
    ...
    with torch.no_grad():
        ...
        # 예측 결과 확인
        _, predicted = torch.softmax(output, dim=1)
...
# 모델 구조를 정의합니다. (예: ResNet18)
model = resnet18(weights=None)  # 또는 weights='DEFAULT'로 설정하여 기본 가중치를 사용할 수 있습니다.

# 저장된 가중치를 모델에 로드합니다.
# model.load_state_dict(torch.load(pklfile, map_location=torch.device('cpu')))
model = torch.load(pklfile, map_location=torch.device('cpu'), weights_only=False)
# 클래스 수에 맞게 마지막 층 수정
# num_classes = 6  # 당신의 클래스 수
...
model.load_state_dict(torch.load(pklfile, map_location=torch.device('cpu'), weights_only=True))
