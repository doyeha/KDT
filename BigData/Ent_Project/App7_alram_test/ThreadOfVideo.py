...
class ThreadOfVideo(QThread, form_class):
    ...
    def run(self):
        if self.video_file:
            ...
            while self.running:    # and (self.frame_number == self.frame_cnt)
                ...
                if self.ret:
                    if self.predicted_frame is not None:
                        ...
                        self.frame_number = cap.get(cv2.CAP_PROP_POS_FRAMES)
                        print(f"if ret: 안에 self.fps : {self.fps}, self.frame_number : {self.frame_number}")
