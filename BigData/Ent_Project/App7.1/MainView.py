...
class MainView(QMainWindow, form_class):
    ...
    def logvideo_opener(self):
        ...
        self.file_path = self.file_path.split("videopath")
        self.file_path = self.file_path[1].replace(" ","")
        print(self.file_path)
        subprocess.run(["start", "", r'C:\Git\KDT\BigData\Ent_Project\App7.1\log_video\test_1211_103848.mp4'], shell=True)
        # pass
    ...
    def reset_drawing_info(self):
            ...
            self.main_width = self.label.width()    # 2050
            self.main_height = self.label.height()
            self.canvas = QPixmap(self.main_width, self.main_height)  # 캔버스 크기 설정
    ...
    def drawing_info_update(self, rec, x1, y1, x2, y2):
        ...
        self.main_width = self.label.width()    # 2050
        self.main_height = self.label.height()  # 1150
        self.canvas = QPixmap(self.main_width, self.main_height)  # 캔버스 크기 설정
        ...
        if rec == "rec1":
            self.rect1_x1 = int(x1 / 2121 * self.main_width)   # drawing의 previewArea 부분 넓이 /rec_file의 넓이
            self.rect1_y1 = int(y1 / 1201 * self.main_height)   # drawing의 previewArea 부분 높이 /rec_file의 높이
            self.rect1_x2 = int(x2 / 2121 * self.main_width)   
            self.rect1_y2 = int(y2 / 1201 * self.main_height)
        if rec == "rec2":
            self.rect2_x1 = int(x1 / 2121 * self.main_width)
            self.rect2_y1 = int(y1 / 1201 * self.main_height)
            self.rect2_x2 = int(x2 / 2121 * self.main_width)
            self.rect2_y2 = int(y2 / 1201 * self.main_height)


        if rec == "rec3":
            self.rect3_x1 = int(x1 / 2121 * self.main_width)
            self.rect3_y1 = int(y1 / 1201 * self.main_height)
            self.rect3_x2 = int(x2 / 2121 * self.main_width)
            self.rect3_y2 = int(y2 / 1201 * self.main_height)
        ...
        if logfile_list:        # log 파일이 있다면
            ...
            if show_file in logfile_list:           # 보여줄파일이있다면
                ...
                with open('log/'+show_file,encoding='utf-8') as f :
                    logs=f.readlines()
                    log_list2=[i.strip('\n').split(' ') for i in logs]
                    log_df=pd.DataFrame(log_list2,columns=['movie','rack','case','date','videopath'])
